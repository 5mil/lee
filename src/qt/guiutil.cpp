...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("lee://"))
    {
        uri.replace(0, 12, "lee:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "lee.lnk";
}
...
{
    return GetAutostartDir() / "lee.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("lee-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  lee-qt [" + tr("command-line options") + "]                     " + "\n";
